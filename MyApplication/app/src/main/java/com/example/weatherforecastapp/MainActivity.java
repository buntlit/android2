...
//        sensorManager = (SensorManager) getSystemService(SENSOR_SERVICE);
    ...
    private void requestLocation() {
        ...
        if (provider != null) {
            locationManager.requestLocationUpdates(provider, 60000, 50, new LocationListener() {
                @Override
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (index == -1)
            return;
        if (requestCode == PERMISSION_REQUEST_CODE && grantResults[index] == PackageManager.PERMISSION_GRANTED)
            requestLocation();
    }
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (id == R.id.sw_city1 || id == R.id.sw_city2 || id == R.id.sw_city3) {
            ...
            clickOrSelect(item.getTitle().toString());
        }
        return super.onOptionsItemSelected(item);
    }


    public static void clickOrSelect(String city) {
        ...
        try {
            Thread.sleep(1500);
            dataSource.addOrEdit(defCity, WeatherFragment.tempCity, WeatherFragment.humCity, WeatherFragment.windCity, WeatherFragment.weatherCity);
